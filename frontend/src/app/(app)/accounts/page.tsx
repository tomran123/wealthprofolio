...
import type { AccountWithNames, HoldingWithInstrument, Institution, Owner } from "@/lib/types";
  ...
  return (
    <Tabs defaultValue="accounts" className="space-y-4">
      <TabsList>
        ...
        <TabsTrigger value="institutions">{t("accounts.tab_institutions")}</TabsTrigger>
...
function AccountsTab() {
  ...
  const ownersQuery = useQuery({ queryKey: ["owners"], queryFn: () => api.get<Owner[]>("/api/owners") });

  const holdingsQuery = useQuery({
    queryKey: ["accounts", expandedId, "holdings"],
    queryFn: () => api.get<HoldingWithInstrument[]>(`/api/accounts/${expandedId}/holdings`),
    enabled: !!expandedId,
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setOpen(false);
      setForm({ name: "", institution_id: "", owner_id: "", account_type: "brokerage", base_currency: "USD" });
    ...
    <div className="space-y-4">
      <div className="flex justify-end">
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger render={<Button>{t("accounts.add_account")}</Button>} />
          <DialogContent>
            <DialogHeader>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t("accounts.select_institution")} />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setForm((f) => ({ ...f, account_type: v ?? "brokerage" }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
            ...
            <DialogFooter>
              <Button
                disabled={!form.name || !form.institution_id || !form.owner_id || createMutation.isPending}
                        ...
                        <TableCell colSpan={5}>
                          <div className="space-y-2 py-2">
                            ...
                            {holdingsQuery.isLoading ? (
                              ...
                              <p className="py-2 text-sm text-muted-foreground">{t("accounts.no_holdings")}</p>
                            ) : (
                              <div className="space-y-1">
                                {(holdingsQuery.data ?? []).map((h) => (
                                  <div key={h.id} className="flex items-center justify-between text-sm">
                                    <span>{h.instrument_symbol ?? h.instrument_name}</span>
                                    <div className="flex items-center gap-2">
                                      <span className="text-muted-foreground">{formatNumber(h.quantity)}</span>
                                      <SetValuationDialog
                                        instrumentId={h.instrument_id}
                                        defaultCurrency={account.base_currency}
                                        trigger={
...
function InstitutionsTab() {
  ...
  const queryClient = useQueryClient();
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({ name: "", institution_type: "bank", country: "" });

  const query = useQuery({ queryKey: ["institutions"], queryFn: () => api.get<Institution[]>("/api/institutions") });
  const createMutation = useMutation({
    mutationFn: () => api.post<Institution>("/api/institutions", { ...form, country: form.country || null }),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger render={<Button>{t("accounts.add_institution")}</Button>} />
          <DialogContent>
            <DialogHeader>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(v) => setForm((f) => ({ ...f, institution_type: v ?? "bank" }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
...
function OwnersTab() {
  ...
  const query = useQuery({ queryKey: ["owners"], queryFn: () => api.get<Owner[]>("/api/owners") });
  const createMutation = useMutation({
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(v) => setForm((f) => ({ ...f, owner_type: v ?? "individual" }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
